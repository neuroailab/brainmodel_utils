...
class PipelineNeuralMap(nm.NeuralMapBase):
    def __init__(self, map_type=None, map_kwargs={}, factor_kwargs=None):
        assert map_type is not None
        self._map_type = map_type
        self._map_kwargs = map_kwargs
        self._factor_kwargs = factor_kwargs
    ...
    def fit(self, X, Y, first_X=None):
        ...
        if self._factor_kwargs is not None:
            self.factor_transform = FactorAnalysis(
                random_state=0, rotation=None, **self._factor_kwargs
            )
            X_in = self.factor_transform.fit_transform(X_in)
    ...
    def predict(self, X):
        ...
        if self._factor_kwargs is not None:
            X_in = self.factor_transform.transform(X_in)
